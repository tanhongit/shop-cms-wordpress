...
<div class="ht-tab-content">
	...
	<div class="shop-items">
		...
		if (1 === intval($total)) {
			...
			printf("<span>" . _n('Showing all %d result', 'Showing all %d results', $total, 'woocommerce') . "</span>", $total);
		} else {
			$first = ($per_page * $current) - $per_page + 1;
			$last  = min($total, $per_page * $current);
			/* translators: 1: first result 2: last result 3: total results */
			printf("<span>" . _nx('Showing %1$d&ndash;%2$d of %3$d result', 'Showing %1$d&ndash;%2$d of %3$d results', $total, 'with first and last result', 'woocommerce') . "</span>", $first, $last, $total);
		}
